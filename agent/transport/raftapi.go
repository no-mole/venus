...
import (
	...
	pb "github.com/no-mole/venus/proto/pbtransport"
...
func (r *grpcTransport) Consumer() <-chan raft.RPC {
	return r.manager.rpcChan
}

// LocalAddr is used to return our local address to distinguish from our peers.
func (r *grpcTransport) LocalAddr() raft.ServerAddress {
	return r.manager.localAddress
}

func (r *grpcTransport) getPeer(id raft.ServerID, target raft.ServerAddress) (pb.RaftTransportClient, error) {
...
func (r *grpcTransport) AppendEntries(id raft.ServerID, target raft.ServerAddress, args *raft.AppendEntriesRequest, resp *raft.AppendEntriesResponse) error {
	c, err := r.getPeer(id, target)
	if err != nil {
		return err
	}
	ret, err := c.AppendEntries(r.ctx, encodeAppendEntriesRequest(args))
	if err != nil {
		return err
	}
...
func (r *grpcTransport) RequestVote(id raft.ServerID, target raft.ServerAddress, args *raft.RequestVoteRequest, resp *raft.RequestVoteResponse) error {
	c, err := r.getPeer(id, target)
	if err != nil {
		return err
	}
	ret, err := c.RequestVote(r.ctx, encodeRequestVoteRequest(args))
	if err != nil {
		return err
	}
...
func (r *grpcTransport) TimeoutNow(id raft.ServerID, target raft.ServerAddress, args *raft.TimeoutNowRequest, resp *raft.TimeoutNowResponse) error {
	c, err := r.getPeer(id, target)
	if err != nil {
		return err
	}
	ret, err := c.TimeoutNow(r.ctx, encodeTimeoutNowRequest(args))
	if err != nil {
		return err
	}
...
func (r *grpcTransport) InstallSnapshot(id raft.ServerID, target raft.ServerAddress, req *raft.InstallSnapshotRequest, resp *raft.InstallSnapshotResponse, data io.Reader) error {
	c, err := r.getPeer(id, target)
	if err != nil {
		return err
	}
	stream, err := c.InstallSnapshot(r.ctx)
	if err != nil {
		return err
	}
...
func (r *grpcTransport) AppendEntriesPipeline(id raft.ServerID, target raft.ServerAddress) (raft.AppendPipeline, error) {
	c, err := r.getPeer(id, target)
	if err != nil {
		return nil, err
	}
...
// send may block which is an effective form of back-pressure.
func (r *raftPipelineAPI) AppendEntries(req *raft.AppendEntriesRequest, _ *raft.AppendEntriesResponse) (raft.AppendFuture, error) {
	af := &appendFuture{
		start:   time.Now(),
		request: req,
...
func (r *grpcTransport) EncodePeer(_ raft.ServerID, addr raft.ServerAddress) []byte {
	return []byte(addr)
}

// DecodePeer is used to deserialize a peer's address.
func (r *grpcTransport) DecodePeer(p []byte) raft.ServerAddress {
...
func (r *grpcTransport) SetHeartbeatHandler(cb func(rpc raft.RPC)) {
